...
- 从BIOS起步，利用一些缺省的寄存器值执行预设的长跳转指令，启动bootloader；
- 先实现一个可显示字符串的bootloader；
- 再实现一个可加载ELF文件的bootloader；
- 最后用bootloader启动操作系统；
...
- 在Linux系统下有一个系统调用表Syscall_table_32.s，里面可以看到系统调用号的对应关系；
- 统计一个系统调用的次数可以在kernel/sys.c构造一个全局变量，所有系统调用的话构造一个全局数组，数组大小可以从上面的调用表中找到。
- 统计次数可以用一个新的系统调用实现，也可以自己写一个函数或SERVICE，打印出所有系统调用的次数。
- 最后在每个系统调用中增加一行汇编代码，每次系统调用后增加对应数组元素的值。
